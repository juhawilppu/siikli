        ...
        <tbody>
          {invoice.items.sort((a, b) => new Date(a.deliveryDate).getTime() - new Date(b.deliveryDate).getTime()).map((item, idx) => (
            <tr key={idx} className="border-bottom">
              ...
              <td className="align-right width-20">{formatNumber(item.quantity)}</td>
              {item.usePrice0
                ? (
                    <>
          ...
          {showTotal && (
            <tr className="border-top border-bottom">
              <td colSpan={3} className="title width-40">
                Yhteensä
                {' '}
                {formatNumber(invoice.totals.totalKg)}
                {' '}
                {invoice.customer.showPriceWithoutTax ? '(ALV 0 %)' : '(ALV 14 %)'}
              </td>
              <td className="align-right">{formatNumber(invoice.totals.totalKg)}</td>
              <td></td>
